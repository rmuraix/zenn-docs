...
DockgeとDockgeが管理するComposeの置き場を作成します。ここではDockgeの`compose.yaml`を`/opt/dockge`に、管理対象（以下スタック）を`/opt/stacks`に配置する例を示しています。管理対象のパスを変更する場合は、後述する`compose.yaml`の記述も変更してください。
